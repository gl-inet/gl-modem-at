...
int32_t uartOpen(int8_t *port, uint32_t baudRate, uint32_t rtsCts, int32_t timeout)
{
    ...
    //    usleep(50000);
    //    tcflush(serialHandle, TCIOFLUSH);       //fd解锁后再刷新缓冲区
...
int32_t MyuartTxNonBlocking(int32_t serialHandle, uint32_t dataLength, const void *data)
{
    /** The amount of bytes written. */
    size_t dataWritten;
    /** The amount of bytes still needed to be written. */
    ...
    while (dataToWrite) {
        dataWritten = write(serialHandle, data, dataToWrite);
        if ((dataWritten == 0) || (dataWritten == -1)) {
            break;
        } else {
            dataToWrite -= dataWritten;
